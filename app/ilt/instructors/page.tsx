...
export default async function IltInstructors() {
  ...
  return (
    <>
      <h1 className="page-title">{T.nav_instructors}</h1>
      <p className="page-sub">{T.ili_sub}</p>

      {instructors.map((ins) => (
        <div className="card" key={ins.id}>
          <h2>{ins.first_name} {ins.last_name}{ins.title ? <span className="muted small"> · {ins.title}</span> : null}</h2>
          <table>
            <thead>
              <tr><th>{T.in_qual}</th><th>{T.in_number}</th><th>{T.in_body}</th><th>{T.in_issued}</th><th>{T.in_validuntil}</th><th>{T.status}</th></tr>
            </thead>
            <tbody>
              {(ins.instructor_certificates ?? []).length === 0 ? (
                <tr><td colSpan={6} className="muted small">{T.in_noqual}</td></tr>
